...
const AddEditBoardModal = ({ setBoardModalOpen, type }) => {
  ...
  return (
    ...
    >
      {/* modal section */}
      <div className="scrollbar-hide overflow-y-scroll max-h-[95vh] bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e] max-w-md mx-auto w-full px-8 py-8 rounded-xl">
        ...
        <div>
          <button
            className="w-full items-center hover:opacity-75 dark:text-[#635fc7] dark:bg-white text-white mt-2 bg-[#635fc7] py-2 rounded-full"
            onClick={() => {
              setNewColumns((state) => [
                ...state,
                { name: "", task: [], id: uuidv4() },
          ...
          >
            {type === "add" ? "Create New Board" : "Save Change"}
          </button>
        </div>
      </div>
    </div>
